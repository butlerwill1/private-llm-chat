...
it('delivers text before completion and maps the saved conversation at done', async () => {
  ...
  wire.feed({ type: 'reasoning', text: 'Considering 世界' })
  wire.feed({ type: 'text', text: 'Hello 世界' })
  await vi.waitFor(() => expect(update).toHaveBeenCalledWith({ type: 'text', text: 'Hello 世界' }))
  expect(update).toHaveBeenCalledWith({ type: 'reasoning', text: 'Considering 世界' })
  wire.feed({ type: 'done', conversation: { id: 'chat/id', title: 'Test', active_model_id: 'model', messages: [{ id: 'answer', role: 'assistant', content: 'Hello 世界', usage: null, reasoning: 'Considering 世界' }] } })
  expect((await result).messages[0]).toMatchObject({ body: 'Hello 世界', reasoning: 'Considering 世界' })
